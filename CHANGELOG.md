...
- Tolerate extra successful or failed control blocks emitted by user `after-*`
  hooks during unattached capture instead of treating a healthy tmux server as
  unavailable or failing the requested sequence. Hook output is excluded from
  snapshot records by the capture format prefix.
...
- Preserve both the original client-switch failure and a subsequent inventory
  failure in restore diagnostics.
